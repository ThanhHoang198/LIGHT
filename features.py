...
def high_saturation_pixels(rgb_image, threshold):
  ...
  avg_green = sum_green / len(high_sat_pixels) * 0.8 # 0.8 to favor xxxx's chances
...
def highest_sat_pixel(rgb_image):
  ...
  content, or a higher xxxx content'''

  hsv = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2HSV)
  s = hsv[:,:,1]

  x, y = (np.unravel_index(np.argmax(s), s.shape))
  if rgb_image[x, y, 0] > rgb_image[x,y, 1] * 0.9: # 0.9 to favor xxxx's chances
...
def red_green_yellow(rgb_image):
  ...
  # Yellow
  lower_yellow = np.array([0,sat_low,val_low])
